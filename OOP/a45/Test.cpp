...
{
	DynamicVector<int> test_vector{ 2 };
	test_vector.add_item(10);
	assert(test_vector.get_size() == 1);
	assert(test_vector.get_element(0) == 10);
	test_vector.update_item(0, 11);
	assert(test_vector.get_element(0) == 11);
	test_vector.delete_item(0);
	assert(test_vector.get_size() == 0);
	test_vector.add_item(9);
	test_vector.add_item(8);
	test_vector.add_item(7);
	assert(test_vector.get_size() == 3);
}
...
void Test::tests_repository()
{
	...
	DynamicVector<Event> events = repository.get_events();
	assert(events.get_size() == 0);
}
...
void Test::test_user_list()
{
	...
	assert(test_list.find_event(event) == 0);
	test_list.delete_event(event);
	assert(test_list.find_event(event) == -1);
	assert(test_list.get_size() == 0);
	assert(test_list.is_empty() == true);
