...
void Ui::print_menu_user()
{
	...
	std::cout << "open - Open file with my list\n";
	std::cout << "exit\n";
}
...
void Ui::add_event_to_array()
{
	...
	for (char const& character : the_date_time)
	{
	...
	for (char const& character : the_number_of_people)
	{
		if (std::isdigit(character) == 0) {
	...
	{
		for (auto the_error : the_exceptions.get_validation_errors())
			std::cout << the_error << '\n';
	}
...
{
	std::vector<Event> list = this->service.get_all_events();
	for (auto event : list)
	{
...
void Ui::print_list_made_by_user(std::vector<Event> list)
{
	if (list.empty())
	{
